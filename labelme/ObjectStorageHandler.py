...
def download_directory_image(bucket_name, img_bucket_name, directory_name, save_path, login_id):
    ...
    not_working = list(set(down_bucket_items_origin) - set(up_nok_bucket_items_origin))  # 검수툴을 이용하는 작업자를 위한 수정
    not_working.sort()
    ...
    progress = QtWidgets.QProgressDialog("Download files...", '', 0, len(items)+1)
    progress.setCancelButton(None)
    progress.setAutoClose(True)
    progress.setWindowModality(Qt.WindowModal)
